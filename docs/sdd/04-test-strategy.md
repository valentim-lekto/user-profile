...
**Status:** aprovada e executada em M1–M6; gates locais pós-M6 de mutação e refinamento visual concluídos · **Data:** 2026-08-28 · **Contrato:** [`03-api-contract.yaml`](03-api-contract.yaml)
...
Baseline observada em 2026-08-27: `S = 97,41%`, com 188 killed, 5 survived, 106 ignored, 2 `CompileError` produzidos por mutações C# inválidas, 0 `NoCoverage`, 0 timeout e 0 erro de execução, em `00:04:41`. O ratchet final é `break/low/high = 97/97/97`. Os cinco survivors equivalentes continuam no relatório, e uma única exclusão pontual justificável continua contabilizada como ignored; não há ignore global.

Como o score considera timeout como detectado, o runner Docker não confia apenas no exit code do Stryker: ele valida o JSON final e reprova `Timeout`, `NoCoverage`, `RuntimeError`, relatório ausente e quantidade de `CompileError` diferente dos dois mutantes inválidos já classificados. Esse gate impede falso-verde sem aumentar timeout, reduzir suíte ou ocultar mutante.
