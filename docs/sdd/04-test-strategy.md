...
|---|---|---|
| `E2E-001` | Cadastrar → ver sucesso no login → autenticar → ver saudação → editar nome/email → consultar novamente dashboard e perfil → encerrar a sessão → tentar reabrir o dashboard. | Caminho feliz principal, persistência de ambos os campos, logout e reproteção. |
...
| `TECH-FRONTEND-001` | `package.json`/lockfile usam Angular standalone/strict, Reactive Forms e Material nas versões fixadas; o Dockerfile confere npm `11.17.0`, e `npm ci` com allowlist estrita e build passam. | `TECH-FRONTEND-01` |
