...
**Status:** M6 e atividades pós-M6, inclusive rate limiting, concluídos localmente; execuções hospedadas pendentes · **Data:** 2026-08-30
...
- **Verified (rate limiting pós-M6):** contrato, frontend e ingresso Nginx foram confrontados com testes estáticos/runtime e rajada HTTP real; não implica lockout, cota distribuída ou persistente.
...
| `FR-UI-01` | `UI-STATE-01`, `UI-RESP-01`, `AC-LOGIN-03`, `AC-DASH-04`, `API-ERROR-02` | Signals e estados em cadastro, login, dashboard e perfil/senha; composição visual pós-M6 | M2–M5, pós-M6 | `FE-REG-001/002`, `FE-LOGIN-001`, `FE-RATE-001`, `FE-DASH-001`, `FE-PROF-001/002`, `FE-PASS-002`, `FE-VISUAL-001`, `E2E-001/002/003` | Verified corrente: 71/71 testes, três jornadas e inspeção real confirmam estados e responsividade; login/cadastro também preservam valores/sessão e encerram loading em `429`, sem navegação ou countdown. |
...
| `NFR-SEC-02` | `SEC-RATE-01`, `SEC-RATE-02`, `API-ERROR-02` | ADR-0005; Nginx antes do proxy genérico; `registerUser`/`loginUser`; alertas de `/register` e `/login` | Hardening pós-M6 | `SPEC-OAS-006`, `BE-OAS-001`, `FE-RATE-001`, `OPS-RATE-001/002` | Verified localmente: contrato estático/runtime, frontend e smoke concorrente confirmaram partições, respostas, não-bypass, reset efêmero, persistência e logs seguros; a correção pós-revisão passou a ignorar diretivas comentadas, exigir headers únicos e tratar o exemplo de `detail` como não normativo. |
| `NFR-TEST-01` | `TEST-FLOW-01` | Estratégia com integração principal, frontend, E2E, Compose, CI e mutation testing crítico do backend; timeout frontend global sem retry | M1–M6, correções e atividades pós-M6 | Gates de `04-test-strategy.md`, `E2E-*`, `CI-001`, `BE-MUT-001`, `CI-MUT-001` | Verified localmente: 121 integrações, 71 testes frontend, três E2E, smoke e baseline Stryker limpa de 97,50% foram aprovados. Execuções hospedadas continuam Pending até publicação e observação. |
...
| `NFR-SDD-02` | `DOC-SDD-01` | ADR-0001 a ADR-0005 | Design e contínuo | Revisão de decisões relevantes | ADR-0005 aceito para o rate limiting local; revisão contínua. |
| `NFR-TRACE-01` | `DOC-TRACE-01` | Este documento e extensões `x-*` do OpenAPI | M1–M6, pós-M6 | `SPEC-TRACE-001` | Verified corrente: 19 requisitos funcionais, 15 não funcionais, 18 premissas e 46 critérios estão mapeados; a atividade de rate limiting foi ligada à evidência executada. |
...
| `PREM-FE-01` | `UI-STATE-01`, `UI-RESP-01` | Angular standalone/strict, Reactive Forms, Material e refinamento visual responsivo | M1–M5, pós-M6 | Build, suíte `FE-*`, `FE-VISUAL-001` e `E2E-*` | Verified corrente: Material/strict/formulários foram preservados; lint, 71/71 testes, build e 3/3 E2E passaram, incluindo geometria de feedback em 320 px. |
...
| Requisito | Critério de aceite | Design / endpoint / tela | Milestone | Teste planejado | Estado |
|---|---|---|---|---|---|
| `NFR-TEST-01` | `TEST-FLOW-01` | Stryker.NET `4.16.0`, allowlist da lógica crítica do backend, suíte xUnit HTTP/EF/SQLite real e ratchet derivado da baseline | Pós-M6 | `BE-MUT-001` | Verified localmente: suíte normal 121/121; baseline corrente com 513 descobertos/200 executados e 97,50% (195 killed, 5 survived, 109 ignored, 3 mutações inválidas em `CompileError`, 0 `NoCoverage`/timeout/erro de execução); ratchet 97/97/97 e relatórios aprovados. |
...
| `FR-UI-01`, `PREM-FE-01` | `UI-RESP-01`, `AC-REG-04`, `AC-PASS-03` | Altura dinâmica dos `mat-form-field` e prioridade do erro próprio da confirmação em `/register` e `/profile` | Correção visual pós-M6 | `FE-REG-001`, `FE-PASS-001`, `FE-VISUAL-001`, `E2E-001` | Verified: as colisões de 8,8 px e 18,4 px foram reproduzidas antes da mudança; lint, 71/71 testes, build e 3/3 E2E passaram. Altura fixa e margem antiga reintroduzidas separadamente fizeram `E2E-001` falhar por 7,8 px e 1,4 px; a inspeção estrutural dos 17 blocos de feedback e a publicação em 320/1280 px não encontraram outro responsável por sobreposição. |

## Atividade de segurança pós-M6 — rate limiting de autenticação

| Requisito | Critério de aceite | Design / endpoint / tela | Milestone | Teste planejado | Estado |
|---|---|---|---|---|---|
| `NFR-SEC-02` | `SEC-RATE-01`, `SEC-RATE-02`, `API-ERROR-02` | ADR-0005; zona Nginx `auth_rate_limit`; `POST /api/auth/register`; `POST /api/auth/login`; `/register`; `/login` | Pós-M6 | `SPEC-OAS-006`, `BE-OAS-001`, `FE-RATE-001`, `OPS-RATE-001`, `OPS-RATE-002` | Verified localmente na atividade de rate limiting em 2026-08-30: contrato, 121 integrações, 70 testes frontend, smoke concorrente, 3 E2E e Stryker passaram; o smoke fortalecido também distinguiu configuração ativa de comentário e tornou headers/corpo semanticamente inequívocos. Limitações local/efêmera permanecem explícitas. |

## Evidência executável do rate limiting pós-M6

| ID | Comando/artefato observado em 2026-08-30 | Estado |
|---|---|---|
| `SPEC-OAS-006`, `BE-OAS-001`, `API-ERROR-02` | Profiles `contract-tests`/`backend-tests`; contrato normativo e Swagger runtime | Verified após correção pós-revisão: 6 operações/56 referências, build Release sem warnings e 121/121 integrações; somente cadastro/login declaram `429 ProblemDetails`, com schema sem composição concorrente, exatamente uma declaração case-insensitive de `Retry-After: 60`/`Cache-Control: no-store`, os cinco campos obrigatórios/não nulos/tipados e `detail` com caractere não branco. Outros erros mantêm referência direta exclusiva aos schemas de erro. |
| `FE-RATE-001`, `UI-STATE-01` | Profile `frontend-tests` | Verified: lint, 70/70 testes e build; mensagem exata, valores e sessão-sentinela preservados, loading encerrado, submit reabilitado e ausência de navegação/countdown. |
| `OPS-RATE-001`, `SEC-RATE-01`, `API-ERROR-02` | Probes sintéticos e `./scripts/validate-m1-compose.sh` | Verified após correção pós-revisão: comentário `10r/m` com `1r/m` ativo, `Retry-After` conflitante, `status: 429.0` e detalhe somente NBSP foram rejeitados; whitespace equivalente e outra copy genérica segura foram aceitos. O inventário real usa somente diretivas ativas e exige `1m/10r/m` mais um único `limit_req`; rajadas independentes de 11 logins e 11 cadastros produziram, cada uma, exatamente 10 respostas da API e um `429`; cada resposta limitada trouxe uma ocorrência de `Retry-After: 60` e `Cache-Control: no-store`, com os cinco campos tipados/não nulos, `detail` contendo caractere não branco e nenhum dado sensível. Outros métodos/rotas e `413` permaneceram fora do limite. |
| `OPS-RATE-002`, `SEC-RATE-02`, `SEC-LOG-01`, `OPS-PERSIST-001` | Smoke com query/XFF/caixa/barra e duas recriações somente de `web` | Verified: nenhum bypass; uma segunda rajada completa de login após recriar `web` comprovou reset integral da cota; API/SQLite foram preservados e os logs não contiveram os marcadores/credenciais observados. |
| `TEST-FLOW-01`, `BE-MUT-001` | 3 E2E e profile `mutation-tests` | Verified: 3/3 E2E; Stryker 97,50%, 195 killed/5 survived equivalentes/109 ignored/3 `CompileError`, sem timeout, `NoCoverage` ou erro. |
...
| `UI-RESP-01`, `FE-REG-001`, `FE-PASS-001`, `FE-VISUAL-001`, `E2E-001` | Profile `frontend-tests`; provas negativas/positiva de `e2e-playwright.sh`; inventário dos 17 feedbacks; navegador publicado | Verified: baseline dos testes falhou 3/71 antes do código; estado final passou lint, 71/71 testes, build e 3/3 E2E em 6,4 s. O oráculo aguarda animações sem pausa fixa e rejeitou altura fixa/margem antiga por 7,8/1,4 px; os 15 feedbacks compartilhados permanecem em fluxo normal, e a inspeção real confirmou separação em 320/1280 px. |
...
- Todos os 19 requisitos funcionais, 15 não funcionais, 18 premissas e 46 critérios permanecem ligados a design, etapa, teste e estado; `NFR-SEC-02`/`SEC-RATE-01/02`/`API-ERROR-02` registram o hardening local de autenticação.
...
- A suíte corrente aprovou 121 integrações backend e 71 testes frontend; as três jornadas E2E, actionlint, Compose config e smoke concorrente passaram em 2026-08-30.
...
- O gate pós-M6 usa Stryker.NET `4.16.0`: 513 mutantes descobertos/200 executados, 195 killed, 5 survivors equivalentes, 109 ignored e 3 `CompileError` classificados; score 97,50%, ratchet 97/97/97, HTML/JSON e zero timeout/`NoCoverage`/erro. A execução hospedada semanal/manual ainda não foi observada.
...
- `UI-RESP-01` está Verified: o E2E existente cobre landscape curto, sequência visual/Tab, nome válido de 200 caracteres e mensagens de validação sem interseção em 320 px; a inspeção dos 17 blocos de feedback confirmou que os demais usam o fluxo compartilhado sem posicionamento ou margem negativos. Profile frontend e publicação real confirmaram o estado sem nova jornada ou lógica de negócio.
- `NFR-SEC-02` está Verified localmente: apenas os dois POSTs públicos consomem cotas independentes por IP/endpoint; `429`, não-bypass, reset efêmero, persistência e feedback frontend foram observados sem middleware backend, lockout ou estado distribuído. O fechamento pós-revisão também eliminou os falsos-verdes de comentário/header e o falso-vermelho da mensagem ilustrativa.
