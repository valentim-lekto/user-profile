...
| `OPS-RATE-001`, `SEC-RATE-01` | `./scripts/validate-m1-compose.sh` | Verified: rajadas independentes de 11 logins e 11 cadastros produziram, cada uma, exatamente 10 respostas da API e um `429`; outros métodos/rotas e `413` permaneceram fora do limite. |
...
- `NFR-SEC-02` está Verified localmente: apenas os dois POSTs públicos consomem cotas independentes por IP/endpoint; `429`, não-bypass, reset efêmero, persistência e feedback frontend foram observados sem middleware backend, lockout ou estado distribuído.
