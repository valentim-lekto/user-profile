...
| `FR-LOGIN-01` | `AC-LOGIN-01`, `AC-LOGIN-02` | DTO `LoginRequest`, `loginUser`, `/login` | M3 | `BE-LOGIN-001/002`, `FE-LOGIN-001` | M3 concluído: login normalizado, validação e credenciais verificadas por integração/frontend/smoke. |
...
| `FR-AUTH-01` | `AC-DASH-02` | JWT middleware, functional guard, rotas `/dashboard` e `/profile` | M3–M4 | `BE-AUTH-001`, `BE-PASS-004`, `FE-GUARD-001`, `E2E-003` | Parcial M3 concluída: autenticação Bearer, guard e rotas protegidas aprovados; autorização dos PUTs de M4 permanece pendente. |
...
| `FR-DASH-01` | `AC-DASH-01`, `AC-DASH-04` | `getCurrentProfile`, `ProfileService`, `/dashboard` | M3 | `BE-PROF-001`, `FE-DASH-001`, `E2E-001` | M3 concluído: dashboard protegido busca a API e mostra o nome; loading/erro foram exercitados na suíte e na UI real. |
...
| `NFR-TECH-02` | `TECH-FRONTEND-01` | Angular standalone/strict, Material, Reactive Forms, signals e lock npm | M1–M4 | `TECH-FRONTEND-001`, build e suíte `FE-*` | Parcial M3 concluída: login/dashboard standalone, signals, lint, 0 vulnerabilidades, 42 testes e build sem warnings aprovados; edição permanece M4. |
...
| `NFR-SEC-01` | `SEC-AUTH-01`, `SEC-SESSION-01`, `SEC-SECRET-01`, `SEC-LOG-01` | ADR-0003, DTOs, logging, allowlist do interceptor e `sessionStorage` | M2–M5 | `BE-AUTH-*`, `BE-CONFIG-001`, `BE-PASS-004`, `BE-DTO-001`, `FE-INT-001`, `OPS-SECRET-001` | Parcial M3 concluída: validação JWT, `sub`, token curto, allowlist, limpeza em `401`, DTO seguro e logs sem marcadores/JWT observado passaram; senha e auditoria final de hash/chave/CI permanecem futuras. |
| `NFR-TEST-01` | `TEST-FLOW-01` | Estratégia com integração principal, frontend, E2E e Compose | M1–M6 | Gates de `04-test-strategy.md` | Parcial M3 concluída: 69 integrações, 42 testes frontend, smoke isolado e UI real cobrem login/proteção/dashboard; E2E completo e CI permanecem M5. |
...
| `PREM-FE-01` | `UI-STATE-01` | Angular standalone/strict, Reactive Forms, Material | M1–M4 | Build e suíte `FE-*` | Parcial M3 aprovada: cadastro/login/dashboard tipados, Material, signals, lint/build e 42 testes passaram; formulários de edição/senha permanecem futuros. |
...
| `PREM-EMAIL-01` | `AC-REG-05`, `AC-PROF-04` | Para emails ASCII aceitos: `Trim().ToUpperInvariant()` nos três fluxos | M2–M4 | `BE-REG-003/004`, `BE-LOGIN-001`, `BE-PROF-005` | Parcial M3: cadastro e login normalizado foram aprovados; edição reutilizará a regra em M4. |
...
| `PREM-AUTH-02` | `AC-DASH-01` | Dashboard usa `getCurrentProfile` | M3 | `FE-DASH-001`, `E2E-001` | M3 concluído: dashboard usa `GET /api/profile`, mostra o nome e foi validado por testes/UI real; E2E completo permanece M5. |
| `PREM-AUTH-03` | `SEC-SESSION-01` | JWT de 15 minutos em `sessionStorage`, sem refresh | M3 | `BE-LOGIN-001`, `FE-LOGIN-002` | M3 concluído: duração, claims, armazenamento, logout e limpeza em `401` aprovados; não há refresh. |
...
|---|---|---|
| `BE-LOGIN-001`, `BE-LOGIN-002` | `AuthenticationTests.cs`: login válido, normalização, payload inválido e credenciais inexistentes/incorretas | Aprovados na suíte de 69 integrações. O token usa `sub`, `jti`, `iat` e `exp` como claims mínimas, além de `iss`/`aud` exigidos para validação; as duas falhas de credenciais devolvem `401 ProblemDetails` byte-idêntico. |
...
| `FE-LOGIN-001`, `FE-LOGIN-002`, `FE-GUARD-001`, `FE-INT-001`, `FE-DASH-001` | Specs Angular de login, autenticação/interceptor, guard e dashboard | 42/42 testes aprovados: formulário/erro/loading, navegação, allowlist/limpeza em `401`, guard, nome retornado pela API, perfil protegido e logout. |
| `TECH-FRONTEND-001` | Imagem Node `24.19.0-bookworm-slim`: `npm ci`, lint, teste e build | 494 pacotes/0 vulnerabilidades; lint e build sem warnings; 42/42 testes; 317,28 kB bruto e 87,60 kB estimado. |
...
| `OPS-COMPOSE-001`, `OPS-ORIGIN-001`, `OPS-PERSIST-001`, `OPS-SECRET-001` | `docker compose config --quiet` e `scripts/validate-m1-compose.sh` | Smoke M1+M2+M3 aprovado em projeto/volume isolados: same-origin, registro/login, `401` equivalente/Bearer, perfil por `sub`, `413/415`, logs sem os marcadores nem o primeiro JWT, recriação API com novo token/persistência, `503` e cleanup. |
...
- A suíte acumulada registra 69 integrações backend e 42 testes frontend aprovados, sem falhas ou skips; contrato normativo, configuração Compose, smoke isolado e UI real também passaram.
- O smoke final de M3 comprovou que a sessão renovada após recriar a API consulta o perfil persistido e que os logs anteriores à recriação não contêm os marcadores sintéticos nem o primeiro JWT. A ausência de hash nas respostas é coberta por `BE-DTO-001`; a auditoria acumulada de hash/token/chave nos logs permanece em M5.
